...
def kos(store, monkeypatch, tmp_path, yanitlar, filters=None):
    ...
    sweeper = LinkSweeper(store.path, {"database": str(store.path)})
    sweeper.start(filters or {"min_score": 0}, by="test")
